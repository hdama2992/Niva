import { ArrowLeft, Check, CircleAlert, UsersRound, X } from 'lucide-react-native';
...
export function ManageEventScreen({
  ...
  const load = async () => {
    try {
      const payload = await listEventMembers(idToken, eventId);
  ...
  return (
    <View style={styles.screen}>
      <View style={styles.topBar}>
        ...
        <Text numberOfLines={1} style={styles.topBarTitle}>Manage event</Text>
        <View style={styles.iconButton} />
      </View>
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.eventTitle}>{event.title}</Text>
        <Text style={styles.subtitle}>Approve requests before the cohort chat opens. Mark attendance after the event.</Text>
        {error ? <Text style={styles.error}>{error}</Text> : null}
        <MemberSection title="Join requests" members={pending} empty="No requests waiting for review." renderActions={(member) => (
          <View style={styles.actions}>
            <SmallAction busy={updatingMemberId === member.id} label="Approve" onPress={() => void update(member, 'APPROVED')} tone="approve" />
            <SmallAction busy={updatingMemberId === member.id} label="Decline" onPress={() => void update(member, 'CANCELLED')} tone="decline" />
          </View>
        )} />
        <MemberSection title="Confirmed members" members={confirmed} empty="No approved members yet." renderActions={(member) => (
          <View style={styles.actions}>
            <SmallAction busy={updatingMemberId === member.id} label="Attended" onPress={() => void update(member, 'ATTENDED')} tone="approve" />
            <SmallAction busy={updatingMemberId === member.id} label="No-show" onPress={() => void update(member, 'NO_SHOW')} tone="decline" />
          </View>
        )} />
        {finished.length ? <MemberSection title="Recorded outcomes" members={finished} empty="" renderActions={() => null} /> : null}
      </ScrollView>
    </View>
  );
}
...
function MemberSection({
  ...
  return (
    <View style={styles.section}>
      ...
      {members.length ? <View style={styles.memberList}>{members.map((member) => <MemberRow key={member.id} member={member}>{renderActions(member)}</MemberRow>)}</View> : <Text style={styles.empty}>{empty}</Text>}
    </View>
  );
}

function MemberRow({ children, member }: { children: React.ReactNode; member: EventMember }) {
  const name = member.user.displayName ?? member.user.username ?? 'Niva member';
  return (
    <View style={styles.member}>
      <View style={styles.memberHeader}>
        <View style={styles.memberIcon}><UsersRound color={colors.info} size={18} strokeWidth={2.3} /></View>
        <View style={styles.memberCopy}>
          <Text style={styles.memberName}>{name}</Text>
          {member.user.profile?.interests?.length ? <Text style={styles.memberInterests} numberOfLines={1}>{member.user.profile.interests.slice(0, 3).join(' · ')}</Text> : null}
        </View>
      </View>
      {children}
    </View>
...
function SmallAction({ busy, label, onPress, tone }: { busy: boolean; label: string; onPress: () => void; tone: 'approve' | 'decline' }) {
  const Icon = tone === 'approve' ? Check : tone === 'decline' ? X : CircleAlert;
  return <Pressable accessibilityRole="button" disabled={busy} onPress={onPress} style={[styles.smallAction, tone === 'approve' ? styles.approveAction : styles.declineAction, busy && styles.busy]}><Icon color={tone === 'approve' ? colors.surface : colors.primaryDark} size={16} strokeWidth={2.5} /><Text style={[styles.smallActionText, tone === 'decline' && styles.declineActionText]}>{label}</Text></Pressable>;
}

const styles = StyleSheet.create({
  actions: { flexDirection: 'row', gap: spacing.sm, marginTop: spacing.md },
  approveAction: { backgroundColor: colors.secondary, borderColor: colors.secondary },
  busy: { opacity: 0.55 },
  content: { padding: spacing.lg, paddingBottom: spacing.xxl },
  count: { color: colors.muted, fontSize: typography.small, fontWeight: '800' },
  declineAction: { backgroundColor: colors.surface, borderColor: colors.primary },
  ...
  eventTitle: { color: colors.ink, fontSize: typography.heading, fontWeight: '800' },
  ...
  smallAction: { alignItems: 'center', borderRadius: radius.md, borderWidth: 1, flex: 1, flexDirection: 'row', gap: 5, justifyContent: 'center', minHeight: 40 },
  smallActionText: { color: colors.surface, fontSize: typography.small, fontWeight: '800' },
