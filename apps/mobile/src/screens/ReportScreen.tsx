...
const reasons: Array<{ id: ReportReason; label: string; text: string }> = [
  { id: 'SPAM', label: 'Spam', text: 'Unwanted promotions or repeated content.' },
  { id: 'FAKE_PROFILE', label: 'Fake profile', text: 'A misleading or impersonated identity.' },
  { id: 'HARASSMENT', label: 'Harassment', text: 'Threatening, pressuring, or unwanted conduct.' },
  { id: 'INAPPROPRIATE_BEHAVIOUR', label: 'Inappropriate behaviour', text: 'Conduct that violates community boundaries.' },
  { id: 'OTHER', label: 'Other', text: 'Something else the moderation team should review.' },
];

export function ReportScreen({ onBack, onSubmit, targetName }: ReportScreenProps) {
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.footer}>
        <Pressable accessibilityRole="button" onPress={() => onSubmit({ details: details.trim() || undefined, reason })} style={styles.submitButton}>
...
const styles = StyleSheet.create({
  ...
  fieldLabel: { color: colors.ink, fontSize: typography.body, fontWeight: '800', marginTop: spacing.xl },
  footer: { backgroundColor: colors.surface, borderTopColor: colors.border, borderTopWidth: 1, padding: spacing.md },
  iconButton: { alignItems: 'center', height: 44, justifyContent: 'center', width: 44 },
  iconWrap: { alignItems: 'center', backgroundColor: colors.accentSoft, borderRadius: radius.pill, height: 56, justifyContent: 'center', width: 56 },
  input: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, color: colors.ink, fontSize: typography.body, lineHeight: 22, marginTop: spacing.sm, minHeight: 132, padding: spacing.md, textAlignVertical: 'top' },
  radio: { borderColor: colors.muted, borderRadius: radius.pill, borderWidth: 1.5, height: 18, marginTop: 2, width: 18 },
  radioSelected: { backgroundColor: colors.primary, borderColor: colors.primary, borderWidth: 5 },
  reason: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, flexDirection: 'row', gap: spacing.sm, padding: spacing.md },
  reasonCopy: { flex: 1 },
  reasonLabel: { color: colors.ink, fontSize: typography.body, fontWeight: '800' },
  reasonList: { gap: spacing.sm, marginTop: spacing.xl },
  reasonSelected: { borderColor: colors.primary, borderWidth: 2 },
  reasonText: { color: colors.muted, fontSize: typography.small, lineHeight: 19, marginTop: 3 },
  ...
  title: { color: colors.ink, fontSize: typography.heading, fontWeight: '800', marginTop: spacing.xl },
